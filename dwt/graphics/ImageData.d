...
static void blit(int op,
    ...
    /*Fast 32 to 32 blit */
    if (alphaMode is 0x10000 && stype is TYPE_GENERIC_32_MSB && dtype is TYPE_GENERIC_32_MSB) {
        if (srcRedMask is 0xFF00 && srcGreenMask is 0xff0000 && srcBlueMask is 0xff000000 && destRedMask is 0xFF0000 && destGreenMask is 0xff00 && destBlueMask is 0xff) {
            for (int dy = destHeight, sfy = sfyi; dy > 0; --dy, sp = spr += (sfy >>> 16) * srcStride, sfy = (sfy & 0xffff) + sfyi, dp = dpr += dpryi) {
                for (int dx = destWidth, sfx = sfxi; dx > 0; --dx, dp += dprxi, sfx = (sfx & 0xffff) + sfxi) {
    ...
    /*Fast 24 to 32 blit */
    if (alphaMode is 0x10000 && stype is TYPE_GENERIC_24 && dtype is TYPE_GENERIC_32_MSB) {
        if (srcRedMask is 0xFF && srcGreenMask is 0xff00 && srcBlueMask is 0xff0000 && destRedMask is 0xFF0000 && destGreenMask is 0xff00 && destBlueMask is 0xff) {
            for (int dy = destHeight, sfy = sfyi; dy > 0; --dy, sp = spr += (sfy >>> 16) * srcStride, sfy = (sfy & 0xffff) + sfyi, dp = dpr += dpryi) {
                for (int dx = destWidth, sfx = sfxi; dx > 0; --dx, dp += dprxi, sfx = (sfx & 0xffff) + sfxi) {
                    destData[dp] = 0;
                    destData[dp + 1] = srcData[sp + 2];
...
static void blit(int op,
    ...
    /*** Fast blit (straight copy) ***/
    if (srcX is 0 && srcY is 0 && destX is 0 && destY is 0 && destWidth is srcWidth && destHeight is srcHeight) {
        if (destDepth is 24 && srcDepth is 8 && (op & BLIT_ALPHA) is 0 && destRedMask is 0xFF0000 && destGreenMask is 0xFF00 && destBlueMask is 0xFF) {
        ...
        if (destDepth is 32 && destOrder is MSB_FIRST && srcDepth is 8 && (op & BLIT_ALPHA) is 0 && destRedMask is 0xFF0000 && destGreenMask is 0xFF00 && destBlueMask is 0xFF) {
