...
bool hasLink(int offset) {
    if (offset is -1) return false;
    ...
    if (event !is null) {
        ...
        if (styles !is null) {
            ...
            if (ranges !is null) {
                ...
                for (int i = 0; i < styles.length; i++) {
                    if (styles[i].start <= offset && offset < styles[i].start + styles[i].length && styles[i >> 1].underline && styles[i >> 1].underlineStyle is DWT.UNDERLINE_LINK) {
                        return true;
                    }
                }
            }
...
void setFont(Font font, int tabs) {
    ...
    if (styledText !is null) {
        ...
        fixedPitch = gc.stringExtent("l").x is gc.stringExtent("W").x; //$NON-NLS-1$ //$NON-NLS-2$
        gc.dispose();
    }
}
