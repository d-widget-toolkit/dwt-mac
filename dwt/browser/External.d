...
Object convertToJava (nsIVariant variant, short type) {
    switch (type) {
        ...
        case nsIDataType.VTYPE_BOOL:
            ...
            if (rc !is XPCOM.NS_OK) Mozilla.error (rc);
            return new Boolean (boolResult[0] !is 0);
        ...
        case nsIDataType.VTYPE_ARRAY:
            ...
            if (rc !is XPCOM.NS_OK) Mozilla.error (rc);
            if (ptr[0] is 0) Mozilla.error (XPCOM.NS_ERROR_NULL_POINTER);
            nsID id = new nsID ();
                ...
                switch (currentType[0]) {
                    ...
                    case nsIDataType.VTYPE_BOOL:
                        ...
                        for (int i = 0; i < count[0]; i++) {
                            ...
                            arrayReturn[i] = new Boolean (boolValue[0] !is 0);
                        }
                        break;
                    case nsIDataType.VTYPE_INT32:
                        arrayReturn = new Object[count[0]];
...
nsIVariant convertToJS (Object value, nsIComponentManager componentManager) {
    ...
    if (value instanceof Boolean) {
        ...
        rc = variant.SetAsBool (boolValue.boolValue () ? 1 : 0);
        if (rc !is XPCOM.NS_OK) Mozilla.error (rc);
        return variant;
    }
